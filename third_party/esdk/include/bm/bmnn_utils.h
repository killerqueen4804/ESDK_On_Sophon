...
#include <string>
#include <memory>
#include <cassert> // 添加此行
#include <set>            // 添加
#include <unordered_map>   // 添加
#include "bmruntime_interface.h"
#include "bmruntime_cpp.h"

#include "bmcv_api.h" // 添加算能图像处理头文件
#include "yolov10.hpp" // 添加
// #include "bm_wrapper.hpp"

/*
 * Any class that inherits this class cannot be assigned.
 */
using YoloV10BoxVec = std::vector<struct YoloV10Box>;
class NoCopyable {
  protected:
    NoCopyable() =default;
    ~NoCopyable() = default;
...
class BMNNNetwork : public NoCopyable {
  ...
  const char* getClassName(int classId) {
        return m_netinfo->output_names[classId]; // 根据实际结构修改
    }
...
class BMNNContext : public NoCopyable {
  ...
  // 新增YOLOv10专用检测接口
    int Detect(const std::vector<bm_image>& inputs, 
              std::vector<YoloV10BoxVec>& outputs) {
        // 1. 设置输入张量
        auto network = this->network(0); // 获取第一个网络
        auto inputTensor = network->inputTensor(0);
        // 修改设备内存设置方式
        bm_device_mem_t mem;
        bm_image_get_device_mem(inputs[0], &mem);  // 正确获取内存
        inputTensor->set_device_mem(&mem);

        // 2. 执行推理
        if(network->forward() != 0) {
            return -1;
        }

        // 3. 获取输出并解析
        auto outputTensor = network->outputTensor(0);
        const float* data = outputTensor->get_cpu_data();
